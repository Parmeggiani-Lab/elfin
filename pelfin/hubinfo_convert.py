...
def main(test_args=None):
    ...
    for i in range(int(len(csv_data)/2)):
        ...
        for i in range(int(len(chains)/2)):
            component_data[chains[int(2*i)]] = { 'single_name': chains[int(2*i+1)], 'c_free': row[4]=='C_free', 'n_free': row[3]=='N_free' }
        new_data[row[0].replace('.pdb', '')] = OrderedDict({ 'oligomer_type': row[1], 'symmetric': row[2] == 'symmetric', 'component_data': component_data})
